...
const BookingConfirmation = () => {
  ...
  const getDriverAllowance = () => {
    const days =
      tripType === "roundtrip" && startDate.length >= 2
        ? Math.max(
            1,
            Math.ceil((new Date(startDate[1]) - new Date(startDate[0])) / (1000 * 3600 * 24))
          )
        : 1;
  ...
  const handleSubmit = async (e) => {
    ...
    const bookingData = {
      ...
      driverAllowance: getDriverAllowance(), // numeric
      permitCharges: 550,
  ...
  const handleConfirm = async (e) => {
    ...
    const timeout = setTimeout(() => {
      setLoading(false);
      toast.error("Booking taking too long. Please try again.", { position: "top-center", autoClose: 4000 });
    }, 8000);
  ...
  const details = [
    ...
    { label: "Driver Allowance", value: formatCurrency(getDriverAllowance()) },
    { label: "Permit Charges", value: "₹ 550" },
    { label: "Toll Charges (Approx)", value: tollCharge ? formatCurrency(tollCharge) : "applicable" },
    { label: "Total", value: formatCurrency(getTotalFare() + getDriverAllowance() + 550 + (tollCharge || 0)) },
  ];
      ...
      <motion.div initial="hidden" animate="visible" variants={fadeUp} className="max-w-4xl mx-auto space-y-10">
        ...
        <motion.div custom={2} variants={fadeUp} className="bg-white shadow-xl rounded-2xl p-6 border border-[#ffc0d1] space-y-4">
          ...
          <div className="bg-[#fff0f5] p-4 rounded-lg text-[#4a1e2d] space-y-1">
            <p>
              <MapPin className="inline w-4 h-4 text-green-600 mr-1" /> <strong>Pickup:</strong> {fromInput}
            </p>
            <p>
              <MapPin className="inline w-4 h-4 text-red-600 mr-1" /> <strong>Drop:</strong> {toInput}
            </p>
          </div>
            ...
            {details.map((item, index) => (
              <motion.div
                ...
                className={`flex justify-between px-4 py-2 rounded-md ${
                  index === details.length - 1 ? "bg-[#ffc0d1] font-bold text-[#4a1e2d]" : "bg-gray-50"
                }`}
              >
        ...
        <motion.form onSubmit={handleSubmit} custom={3} variants={fadeUp} className="bg-white shadow-xl rounded-2xl p-8 border border-[#ffc0d1] space-y-6">
          ...
          <div className="text-center pt-6">
            <button
              ...
              className={`text-white text-lg px-8 py-3 rounded-xl shadow-lg transition-all flex items-center justify-center gap-2 ${loading ? "bg-green-600 cursor-not-allowed" : "bg-[#ff1d58] hover:bg-[#e0144d]"}`}
              disabled={loading}
            >
              {loading ? (
                <>
        ...
        <motion.div custom={4} variants={fadeUp} className="text-center text-[#4a1e2d] mt-10">
          ...
          <p className="text-lg">
            Thank you for choosing <strong>Pallaku Cab Service</strong>. Your ride will be safe, comfortable, and timely. We’ll contact you shortly to confirm your trip.
          </p>
        </motion.div>
      </motion.div>
    </div>
